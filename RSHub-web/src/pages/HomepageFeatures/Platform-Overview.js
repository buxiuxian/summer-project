    ...
    <div className={clsx('col col-4')}>
      ...
      */}
      ...
      <div className="text--left">
        ...
        <div className={styles.center_container}>
            <div className={styles.subhead} style={{marginTop: "20px", marginBottom: "30px", display: 'flex', flexDirection: "row"}}>
              ...
              <div style={{color: "lightgreen", marginRight: "6px" }}> {translate({id: 'homepage.multiple', message: 'Multiple'})} </div> 
            ...
            <div className={styles.center_container_sscenario}>
              ...
              <div className={styles.scenario_cont} onClick={handleRedirect1} style={{ cursor: 'pointer' }}>
                <div className={styles.sscenario_subbg}>
                  <div className={styles.sscenario_submarg}>
        ...
        <Heading as="h3" style={{fontSize: "38px", display: "flex", flexDirection: "row"}}>
          <div style={{color: "green", marginRight: "12px"}}>{translate({id: 'homepage.easier', message: 'Easier'})}</div> 
          {translate({id: 'homepage.to', message: 'to'})}
          <div style={{color: "rgb(14, 6, 156)", marginRight: "12px", marginLeft: "12px"}}>{translate({id: 'homepage.run', message: 'Run'})}</div> 
          {translate({id: 'homepage.and', message: 'and'})}
          <div style={{color: "rgb(255, 95, 5)", marginRight: "12px", marginLeft: "12px"}}>{translate({id: 'homepage.analyze', message: 'Analyze'})}</div>
        </Heading>
        <p>{translate({id: 'homepage.easierDesc', message: 'RSHub makes it easy to run and analyze scenarios of your interests'})}</p>

        <div className={styles.left_cont} style={{marginBottom: "1.2rem"}}>

          <div className={`${styles.Three_step1} ${isHovered1 ? styles.three_hovered : ''}`} onMouseEnter={() => setIsHovered1(true)} onMouseLeave={() => setIsHovered1(false)} >
          <img className={`${styles.Three_step1} ${isHovered1 ? styles.three_hovered : ''}`} src={isHovered1 ? three_step[0] : three_step[3]}/>
          </div>

          <div style={{width: "50vw", marginLeft: "1.5vw"}}>
            <div>{translate({id: 'homepage.describe', message: 'Describe: describe scenario, observation information, and algorithms to model the scenario.'})}</div>
            <ol>
              <li>{translate({id: 'homepage.scenarioDesc', message: 'Scenario description: Define your own research problem by specifying types of scenarios, and corresponding properties of scatters.'})}</li>
              <li>{translate({id: 'homepage.observationDesc', message: 'Observation description: Configure the microwave payload information include observation types (i.e., active or passive), observation angle, and microwave frequencies.'})}</li>
              <li>{translate({id: 'homepage.advancedAlgorithms', message: 'Advanced algorithms-specific parameters: Specify approach to model microwave scattering characteristics including the radiative transfer theory-based and full wave analysis-based options.'})}</li>
            </ol>
          </div>
        </div>

        <div className={styles.right_cont} style={{marginBottom: "1.2rem"}}>
          <div className={styles.right_cont} style={{width: "50vw", marginRight: "1.5vw"}}>{translate({id: 'homepage.deploy', message: 'Deploy: Submit job online for your scenario.'})}</div>
          
          <div className={`${styles.Three_step1} ${isHovered2 ? styles.three_hovered : ''}`} onMouseEnter={() => setIsHovered2(true)} onMouseLeave={() => setIsHovered2(false)}>
          <img className={`${styles.Three_step1} ${isHovered2 ? styles.three_hovered : ''}`} src={isHovered2 ? three_step[1] : three_step[4]}/>
          </div>
        </div>

        <div className={styles.left_cont}>
          <div className={`${styles.Three_step1} ${isHovered3 ? styles.three_hovered : ''}`} onMouseEnter={() => setIsHovered3(true)} onMouseLeave={() => setIsHovered3(false)}>
          <img className={`${styles.Three_step1} ${isHovered3 ? styles.three_hovered : ''}`} src={isHovered3 ? three_step[2] : three_step[5]}/>
          </div>
          <div style={{marginLeft: "1.5vw", width:"50vw"}}>{translate({id: 'homepage.analyzeStep', message: 'Analyze: Analyze your results with analysis tools.'})}</div>
        </div>
      ...
      <div className={styles.center_container} style={{marginTop: "3rem", marginBottom: "3rem"}}>
        <div className={styles.left_cont} style={{backgroundColor: "rgba(134, 217, 255, 0.567)", width: "80vw", height: "100%", minHeight: "25vw", borderRadius:"1rem"}}>
          
          <div style={{margin: "2rem"}}>
            <div style={{fontSize: "3rem", fontWeight: "bold"}}>
              {translate({id: 'homepage.gettingStarted', message: 'Getting Started'})}
            </div>

            <div style={{fontSize: "1.25rem"}}>
              {translate({id: 'homepage.gettingStartedDesc', message: 'We provide a step-by-step guide for a quick start'})}
            </div>

            <div style={{display: "flex", flexDirection: "row"}}>
            <Link to="/Getting-Started" className={styles.get_started_button1}>
              {translate({id: 'homepage.getStarted', message: 'Get started'})}
            </Link>

            <div className={styles.get_started_button2} onClick={handletoWebUsage}>
              {translate({id: 'homepage.webUsage', message: 'Web Usage'})}
            </div>
            </div>
      ...
      {/* End Credit Part */}
      <div className={styles.end_credit_wrapper}>
        ...
        <div className={styles.end_credit_content}>
          ...
          <div style={{display: "flex", flexDirection: "column", marginRight: "3vw"}}>
            ...
            <div style={{marginLeft: "70px", display: "flex", flexDirection: "column"}}>
              <li>{translate({id: 'homepage.publications', message: 'Publications'})}</li>
              <li>{translate({id: 'homepage.references', message: 'References'})}</li>
              <li>{translate({id: 'homepage.presentations', message: 'Presentations'})}</li>
              <li>{translate({id: 'homepage.codeExamples', message: 'Code Examples'})}</li>
              <li>{translate({id: 'homepage.qa', message: 'Q&A'})}</li>
            </div>
          </div>

          <div style={{display: "flex", flexDirection: "column", marginRight: "3vw"}}>
            <div className={styles.subheadani_endnoan} 
            style={{marginTop: "20px", marginLeft: "50px", display: "flex", flexDirection: "column"}}
            >
              {translate({id: 'homepage.contactUs', message: 'Contact Us'})}
            </div>
            <div style={{marginLeft: "50px", display: "flex", flexDirection: "column"}}>
              <div>E-mail: rshub@intl.zju.edu.cn</div>
              <div>ZJUI website: https://zjui.intl.zju.edu.cn/en</div>
            </div>
          </div>
          ...
          <div style={{display: "flex", flexDirection: "column"}}>
            ...
            <div style={{marginLeft: "70px", display: "flex", flexDirection: "column"}}>
              <li>{translate({id: 'homepage.cooperation', message: 'Cooperation'})}</li>
              <li>{translate({id: 'homepage.fundingSupport', message: 'Funding Support'})}</li>
              <li>{translate({id: 'homepage.computingResourcesSupport', message: 'Computing Resources Support'})}</li>
              <li>{translate({id: 'homepage.projectSupport', message: 'Project Support'})}</li>
            </div>
          </div>
        </div>
      </div>
